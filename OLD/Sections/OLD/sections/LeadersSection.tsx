...
export default function LeadersSection({ leaders }: { leaders: Leader[] }) {
    const [activeIdx, setActiveIdx] = useState(0)
    const [isPaused, setIsPaused] = useState(false)
    const scrollContainerRef = useRef<HTMLDivElement>(null)

    const personnel: UnifiedLeader[] = useMemo(() => leaders.map((l) => ({
        id: String(l.id),
    ...
    const current = personnel[activeIdx]

    const nextSlide = useCallback(() => {
        setActiveIdx((prev) => (prev + 1) % personnel.length)
    }, [personnel.length])
        ...
        >
            <header className="relative z-[60] flex-none h-20 lg:h-24 border-b border-zinc-100 bg-white flex items-stretch">
                ...
                <div className="flex items-stretch bg-zinc-50 border-l border-zinc-100">
                    <button onClick={() => setActiveIdx((prev) => (prev - 1 + personnel.length) % personnel.length)} className="w-12 lg:w-16 flex items-center justify-center border-r border-zinc-100 text-zinc-300 hover:text-black transition-all">
