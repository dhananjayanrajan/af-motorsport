...
export default async function RacingOperationsWorkflow({ req }: WidgetServerProps) {
  const card = {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
      ...
      {WORKFLOWS.map((workflow) => (
        <div key={workflow.title} style={{ ...card, marginBottom: '2px' }}>
          <div style={sectionTitle}>{workflow.title}</div>
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', flexShrink: 0 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', width: '80px' }}>
                    ...
                    {step.collection && (
                      <Link
                        href={`/admin/collections/${step.collection}`}
                        style={{
                          fontSize: '9px',
                          fontWeight: 600,
                          color: 'var(--theme-elevation-400)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
              {workflow.steps.map((step, i) => (
                <div key={i} style={{
                  ...
                  padding: '7px 0',
                  borderBottom: i === workflow.steps.length - 1 ? 'none' : '1px solid var(--theme-elevation-100)',
                }}>
                  <span style={{
                    fontSize: '10px', fontWeight: 700,
                    color: 'var(--theme-elevation-400)',
                    width: '16px', textAlign: 'right', flexShrink: 0, marginTop: '1px',
                  }}>
                    {i + 1}
                  </span>
                  <span style={{ fontSize: '12px', fontWeight: 500, color: 'var(--theme-elevation-700)', width: '140px', flexShrink: 0 }}>
                    {step.label}
                  </span>
                  <span style={{ fontSize: '12px', color: 'var(--theme-elevation-500)', flex: 1 }}>
                    {step.description}
                  </span>
                  {step.collection && (
                    <Link
