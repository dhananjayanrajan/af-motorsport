...
const getStage = (kit: any): { stage: Stage; index: number; missing: string[] } => {
  ...
  if (!kit.contexts?.entities) {
    missing.push('Driver or member assignment')
    return { stage: 'Entity Assignment', index: 4, missing }
  }
  if (!kit.assets?.gallery) {
    missing.push('Photo gallery')
    return { stage: 'Gallery', index: 5, missing }
  }
  if (!kit.assets?.visualizations) {
    missing.push('Technical visualization')
    return { stage: 'Visualization', index: 6, missing }
  }
  return { stage: 'Complete', index: 7, missing: [] }
}

export default async function KitsWorkflow({ req }: WidgetServerProps) {
  const { payload } = req
  ...
  const completedCount = rows.filter(r => r.stage === 'Complete').length
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 0 20px 0' }}>
        ...
        <Link
          href="/admin/collections/kits/create"
          style={{
            fontSize: '12px',
            fontWeight: 500,
          ...
          {STAGES.map((stage, i) => (
            <div key={stage} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', position: 'relative', zIndex: 1 }}>
              <div style={{
                ...
                outline: '1px solid var(--theme-elevation-900)',
              }} />
              <span style={{ fontSize: '9px', color: 'var(--theme-elevation-500)', textAlign: 'center', lineHeight: 1.3 }}>
                {stage}
              </span>
            </div>
          ))}
      ...
      {rows.length === 0 ? (
        ...
        rows.map(row => (
          <Link key={row.id} href={`/admin/collections/kits/${row.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            <div style={{
              ...card,
              borderLeft: `3px solid ${row.stage === 'Complete' ? 'var(--theme-success-500)' : 'var(--theme-elevation-800)'}`,
              padding: '16px 20px',
            }}>
              {/* Kit header */}
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '10px', fontSize: '11px', color: 'var(--theme-elevation-400)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{
                    ...
                    color: row.stage === 'Complete' ? 'var(--theme-success-500)' : 'var(--theme-elevation-600)',
              ...
              <div style={{ width: '100%', height: '2px', background: 'var(--theme-elevation-100)', borderRadius: '2px', overflow: 'hidden', marginBottom: '12px' }}>
                <div style={{
                  width: `${row.progress}%`, height: '100%', borderRadius: '2px',
                  background: row.stage === 'Complete' ? 'var(--theme-success-500)' : 'var(--theme-elevation-800)',
                }} />
              </div>
                ...
                {STAGES.map((_, i) => (
                  <div key={i} style={{ flex: 1, display: 'flex', justifyContent: 'center', position: 'relative', zIndex: 1 }}>
                    <div style={{
                      ...
                      background: stageColor(i, row.index),
                      border: i === row.index ? '2px solid var(--theme-elevation-0)' : 'none',
                      outline: i === row.index ? `1px solid var(--theme-elevation-900)` : 'none',
                    }} />
                  </div>
                ))}
              </div>
              ...
              {row.missing.length > 0 && (
                <div style={{
                  ...
                  {row.missing.map((m, i) => (
                    <span key={i} style={{
                      fontSize: '11px', color: 'var(--theme-elevation-600)',
