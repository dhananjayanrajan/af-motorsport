...
const TimelineSection: React.FC<TimelineSectionProps> = ({
  ...
  return (
    <section id={id} className="relative w-full h-screen bg-background flex flex-col border-b-[3px] border-black-pure overflow-hidden">
      ...
      <div className="flex-1 relative bg-white-pure overflow-hidden">
        <div className="absolute left-0 right-0 bottom-24 md:bottom-32 h-[3px] bg-black-pure z-0" />
        ...
        >
          <div className="flex h-full min-w-max relative pt-12 md:pt-20">
            {timelineDays.map((day, idx) => {
              ...
              return (
                <div key={idx} className="relative w-20 md:w-32 xl:w-40 h-full flex flex-col border-r-[3px] border-neutral-200">
                  {isFirstOfMonth && (
                  ...
                  <div className="absolute bottom-16 md:bottom-24 left-1/2 -translate-x-1/2 flex flex-col items-center z-10">
                    <div className="w-[3px] h-6 md:h-10 bg-black-pure mb-2 md:mb-4" />
                    ...
                    {activeEvents.map((event) => {
                      ...
                      return (
                        <div key={event.id} className="relative flex flex-col items-center">
                          ...
                          >
                            <div className={`w-56 md:w-80 p-0 transition-all duration-500 border-[3px] border-black-pure shadow-[8px_8px_0px_0px_#000000] md:shadow-[12px_12px_0px_0px_#000000] overflow-hidden ${hoveredEvent === event.id ? 'bg-primary-500 -translate-y-4 md:-translate-y-8 shadow-[12px_12px_0px_0px_#000000] md:shadow-[20px_20px_0px_0px_#000000]' : 'bg-white-pure'}`}>

                              <div className="flex justify-between items-center p-3 md:p-4 border-b-[3px] border-black-pure bg-neutral-50 group-hover/card:bg-primary-600 transition-colors">
                                <span className="font-mono text-xs font-black text-black-pure px-2 py-0.5 bg-white-pure border-[3px] border-black-pure shadow-[4px_4px_0px_0px_#000000] uppercase tracking-tighter">
                                  {event.code || 'EVT'}
                                </span>
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className={`size-2.5 md:size-3 rounded-none rotate-45 border-[3px] border-black-pure ${event.status === 'active' ? 'bg-primary-500 animate-pulse' : 'bg-neutral-300'}`} />
                                </div>
                              </div>

                              {event.image && (
                                <div className="relative aspect-[16/10] w-full overflow-hidden border-b-[3px] border-black-pure grayscale hover:grayscale-0 transition-all duration-700">
                                  <Image
                                    src={event.image}
                                    alt={event.title}
                                    fill
                              ...
                              <div className="p-3 md:p-5">
                                ...
                                <div className="flex items-center justify-between pt-3 border-t-[3px] border-black-pure/10 group-hover/card:border-white-pure/20">
                                  ...
                                  <div className="size-8 md:size-10 bg-black-pure text-white-pure flex items-center justify-center group-hover/card:bg-white-pure group-hover/card:text-primary-600 transition-all shadow-[4px_4px_0px_0px_#000000] group-hover/card:shadow-none group-hover/card:translate-x-[4px] group-hover/card:translate-y-[4px]">
                          ...
                          <div className={`w-[3px] transition-all duration-500 origin-bottom bg-black-pure ${hoveredEvent === event.id ? 'h-12 md:h-20 bg-primary-500' : 'h-6 md:h-10 opacity-30'}`} />
                          <div className={`size-3 md:size-4 border-[3px] border-black-pure rotate-45 transition-all duration-500 ${hoveredEvent === event.id ? 'bg-primary-500 scale-125' : 'bg-white-pure'}`} />
                        </div>
                      )
                    })}
                  </div>
      ...
      <div className="min-h-24 md:min-h-32 bg-white-pure border-t-[3px] border-black-pure flex divide-x-[3px] divide-black-pure shrink-0">
