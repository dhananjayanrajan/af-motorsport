...
const HeroMedia: React.FC<HeroMediaProps> = ({ id, title, meta, image, tags, ctaLabel, ctaHref }) => {
        ...
        <section className="relative w-full min-h-screen bg-white-pure flex flex-col overflow-hidden">
            <div className="flex h-16 md:h-20 border-b border-black-pure divide-x divide-black-pure bg-white-pure z-30 sticky top-0">
                <div className="w-16 md:w-20 flex items-center justify-center bg-primary-500 hover:bg-secondary-500 transition-colors duration-300 group">
                    <div className="w-4 h-4 md:w-6 md:h-6 bg-black-pure animate-pulse group-hover:scale-150 transition-transform duration-300" />
                </div>
                ...
                <div className="hidden lg:flex items-center px-4 md:px-8 gap-2 md:gap-4 bg-neutral-50">
                    {tags.map((tag, i) => (
                        <span key={i} className="font-mono text-[8px] md:text-[10px] font-bold uppercase border border-black-pure px-2 py-1 hover:bg-black-pure hover:text-white-pure transition-all duration-300 cursor-default">
                            {tag}
                        </span>
                    ))}
                </div>
            ...
            <div className="flex-1 flex flex-col lg:flex-row relative min-h-[calc(100vh-4rem)]">
                <div className="w-full lg:w-[480px] xl:w-[560px] border-r border-black-pure bg-white-pure p-6 md:p-8 lg:p-12 flex flex-col justify-between z-20">
                    <div>
                        ...
                        <h1 className="font-race text-5xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl text-black-pure uppercase leading-[0.85] tracking-tighter break-words">
                            {title}
                        </h1>
                    </div>

                    <div className="space-y-6 md:space-y-10 mt-8 md:mt-0">
                        <div className="relative p-6 md:p-8 border-2 md:border-4 border-black-pure bg-white-pure shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] md:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(236,72,153,0.5)] transition-shadow duration-300">
                            <p className="font-mono text-sm md:text-base font-black uppercase leading-tight text-black-pure">
                                {meta}
                            </p>
                        </div>
                        ...
                        <div className="flex items-center justify-between font-mono text-[8px] md:text-[10px] font-bold text-neutral-400">
                            <div className="space-y-1">
                                <p>IDENTITY // {id}</p>
                                <p>STAMP // {new Date().getFullYear()}</p>
                            </div>
            ...
            <SectionScroller items={[id, title, "EXPLORE", "DISCOVER", "CONNECT"]} variant={3} velocity={25} />
        </section>
    )
}
