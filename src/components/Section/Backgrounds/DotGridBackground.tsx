    ...
    const draw = () => {
      ...
      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          const x = j * spacing
          const y = i * spacing + Math.sin(time * 0.5 + i * 0.4 + j * 0.2) * 6
          const wave = Math.sin(time * 0.8 + i * 0.5 + j * 0.3) * 0.5 + 0.5
          const color = (i + j) % 2 === 0 ? primaryColor : secondaryColor
          const alpha = (0.2 + wave * 0.2) * opacity
          const size = 2.5 + wave * 2
