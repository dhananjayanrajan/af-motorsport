...
const SectionHeader: React.FC<SectionHeaderProps> = ({ title, subtitle, variant = 1, championships = [] }) => {
    ...
    const featuredAbbr = championships.length > 0 ? championships[0].basics?.identifiers?.abbreviation : 'GP'

    if (variant === 2) {
        return (
            <header className="w-full h-24 shrink-0 bg-black-pure relative z-30 flex items-center px-8 border-b-2 border-primary">
                <div className="flex flex-col">
                    <span className="text-primary font-mono text-[10px] font-black tracking-[0.5em] uppercase">{totalItems} TOTAL ENTRIES</span>
                    <h1 className="text-3xl font-black uppercase tracking-tighter text-white-pure">{title}</h1>
                </div>
                <div className="mx-8 h-12 w-px bg-white-pure/20 rotate-12" />
                <div className="flex flex-col">
                    <span className="text-secondary font-mono text-[10px] font-black tracking-[0.5em] uppercase">OFFICIAL</span>
                    <h2 className="text-xl font-black uppercase text-white-pure/60">{subtitle}</h2>
                </div>
                <div className="ml-auto hidden md:flex items-center gap-6">
                    <div className="flex flex-col text-right">
                        <span className="text-white-pure font-mono text-[10px] font-black opacity-40 uppercase">SEASON</span>
                        <span className="text-primary font-black text-sm uppercase">OPEN CLASS</span>
                    </div>
    ...
    if (variant === 3) {
        return (
            <header className="w-full h-24 shrink-0 bg-white-50 relative z-30 flex flex-col justify-between border-b-2 border-black-pure">
                <div className="flex-1 flex items-center px-8 justify-between">
                    ...
                    <div className="font-mono text-[10px] font-black space-x-6 hidden sm:block">
                        <span className="text-black-pure/40">ENTRY NO: {totalItems}</span>
                        <span className="text-black-pure">FEATURED: {featuredAbbr}</span>
                    </div>
                </div>
    ...
    if (variant === 4) {
        return (
            <header className="w-full h-24 shrink-0 bg-tertiary-500 relative z-30 flex items-stretch border-b-2 border-black-pure">
                ...
                <div className="flex-1 flex flex-col justify-center px-12">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-mono text-[9px] font-black uppercase text-black-pure/60">CHAMPIONSHIP DIRECTORY</span>
                    </div>
                    <h1 className="text-4xl font-black uppercase tracking-tighter text-black-pure italic leading-none">{title}</h1>
                </div>
    ...
    if (variant === 5) {
        return (
            <header className="w-full h-24 shrink-0 bg-white-100 relative z-30 grid grid-cols-12 border-b-2 border-black-pure">
                <div className="col-span-12 lg:col-span-8 flex items-center px-10 border-r-2 border-black-pure">
                    <div className="w-2 h-12 bg-secondary mr-6" />
                    <div className="flex flex-col">
                        <h1 className="text-3xl font-black uppercase tracking-widest text-black-pure leading-none">{title}</h1>
                        <span className="font-mono text-[10px] font-black text-black-pure/40 uppercase mt-2 tracking-widest">{totalItems} SERIES DISCOVERED</span>
                    </div>
                </div>
    ...
    return (
        <header className="w-full h-24 shrink-0 bg-secondary-50 relative z-30 flex">
            ...
            <div className="hidden md:flex items-center px-6 font-mono text-xs tracking-widest text-foreground shrink-0 font-black gap-4">
                <div className="text-right">
                    <span className="block text-secondary opacity-50">REGISTRY</span>
                    <span>{featuredAbbr}</span>
                </div>
                <div className="w-px h-8 bg-black-pure/10" />
                <div className="text-right">
                    <span className="block text-primary opacity-50">ACTIVE</span>
                    <span>{year}</span>
                </div>
            </div>
        </header>
