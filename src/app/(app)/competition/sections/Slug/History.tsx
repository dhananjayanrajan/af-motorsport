...
export default function SlugHistory({
    ...
    const defaultImage = `https://picsum.photos/seed/${id}/800/1200`;
    const displayImage = coverImage || defaultImage;
    const displayYear = establishedYear || '2026';
    const displayCategory = category || 'PREMIER';
    const displayProtocol = protocol || 'STANDARD';
    ...
    const historyChildren = historyContent?.root?.children;
            ...
            <div className="max-w-6xl mx-auto px-6 relative z-10">
                ...
                <motion.div style={{ opacity: smoothOpacity }} className="max-w-3xl mx-auto">
                    <div className="grid grid-cols-3 gap-3 md:gap-6 mb-14 pb-6 border-b border-black-pure/20">
                        {[
                            { label: 'ESTABLISHED', val: displayYear, icon: Timer, color: 'bg-primary-500' },
                            { label: 'CLASS', val: displayCategory, icon: Award, color: 'bg-secondary-500' },
                            { label: 'RULES', val: displayProtocol, icon: Globe, color: 'bg-tertiary-500' },
                        ].map((item, i) => (
