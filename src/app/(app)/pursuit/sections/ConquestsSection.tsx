          ...
          <div className="space-y-6">
            ...
            >
              <motion.div
                ...
                className="h-[1px]"
              />
              <span className={cn("text-[8px] md:text-[10px] font-black italic text-zinc-500 uppercase")}>Tactical Overview</span>
            </motion.div>
          ...
          >
            ...
            <MapMarker longitude={HQ.lng} latitude={HQ.lat}>
              <MarkerContent className="size-10 flex items-center justify-center">
                <div className="absolute inset-0 rounded-full animate-pulse scale-150" style={{ backgroundColor: DESIGN_SYSTEM.COLORS.PRIMARY_GLOW, opacity: 0.2 }} />
                <Navigation2 className="size-5" style={{ color: DESIGN_SYSTEM.COLORS.PRIMARY, fill: DESIGN_SYSTEM.COLORS.PRIMARY, filter: `drop-shadow(0 0 8px ${DESIGN_SYSTEM.COLORS.PRIMARY_GLOW})` }} />
              </MarkerContent>
              <MarkerLabel position="bottom" className="text-[7px] font-black text-white px-2 py-1 mt-2 tracking-tighter bg-red-600">BASE_STATION</MarkerLabel>
            </MapMarker>

            {filteredCircuits.map((circuit) => (
              <MapMarker key={circuit.id} longitude={circuit.lng} latitude={circuit.lat} onClick={() => setActiveCircuit(circuit)}>
                <MarkerContent className="cursor-pointer">
                  <motion.div
                    animate={{ scale: activeCircuit?.id === circuit.id ? 1.3 : 1, borderColor: activeCircuit?.id === circuit.id ? "#ffffff" : DESIGN_SYSTEM.COLORS.PRIMARY, backgroundColor: activeCircuit?.id === circuit.id ? DESIGN_SYSTEM.COLORS.PRIMARY : "#000000" }}
                ...
                <MarkerPopup closeButton={false} className="p-0">
                  <motion.div initial={{ opacity: 0, scale: 0.9, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="w-[300px] bg-black border-2 border-white/10 p-0 overflow-hidden shadow-[0_25px_50px_-12px_rgba(0,0,0,0.8)]">
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="text-4xl font-black italic leading-none" style={{ color: DESIGN_SYSTEM.COLORS.PRIMARY, filter: `drop-shadow(0 0 10px ${DESIGN_SYSTEM.COLORS.PRIMARY_GLOW})` }}>{circuit.result}</div>
                      </div>
                      ...
                      <p className="text-[10px] font-bold text-zinc-300 uppercase leading-relaxed italic border-l-2 pl-4 py-1" style={{ borderLeftColor: DESIGN_SYSTEM.COLORS.PRIMARY }}>{circuit.details}</p>
                    </div>
                  </motion.div>
                </MarkerPopup>
              </MapMarker>
          ...
          <AnimatePresence>
            {showRoutes && routes.length > 0 && (
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="absolute top-4 left-4 z-20 flex flex-col gap-2">
                <div className="bg-black/80 backdrop-blur-md border p-4 space-y-4 min-w-[200px]" style={{ borderColor: DESIGN_SYSTEM.COLORS.PRIMARY }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 md:gap-4">
          {[
            ...
            <motion.button
              ...
              className={cn("flex flex-col p-8 border-t-2 transition-all duration-500 text-left group relative overflow-hidden", (filter === stat.id || (stat.id === 'ROUTES' && showRoutes)) ? 'bg-zinc-950' : 'border-zinc-900 hover:border-zinc-700 bg-black')}
              style={{ borderTopColor: (filter === stat.id || (stat.id === 'ROUTES' && showRoutes)) ? DESIGN_SYSTEM.COLORS.PRIMARY : undefined }}
            >
              <div className="flex items-center justify-between mb-6 relative z-10">
                <stat.icon className="size-4" style={{ color: (filter === stat.id || (stat.id === 'ROUTES' && showRoutes)) ? DESIGN_SYSTEM.COLORS.PRIMARY : '#3f3f46' }} />
                <span className="text-[7px] font-mono text-zinc-600 uppercase tracking-[0.3em]">Module_0{i + 1}</span>
              </div>
              <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1 group-hover:text-white transition-colors relative z-10">{stat.label}</span>
              <div className={cn("text-4xl font-black italic tracking-tighter transition-colors duration-500 relative z-10", (filter === stat.id || (stat.id === 'ROUTES' && showRoutes)) ? 'text-white' : 'text-zinc-800')}>{stat.value}</div>
            </motion.button>
          ))}
        </div>
      </div>
