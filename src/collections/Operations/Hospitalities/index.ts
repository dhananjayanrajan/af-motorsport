...
export const Hospitalities = collectionFactory(
  {
    slug: 'hospitalities',
    ...
    access: {
      ...
      create: ({ req: { user } }) => {
        ...
        return user.roles.some((role) => ['admin', 'race_admin', 'hospitality_manager'].includes(role))
      },
      update: ({ req: { user } }) => {
        if (!user || !('roles' in user) || !user.roles) return false
        return user.roles.some((role) => ['admin', 'race_admin', 'hospitality_manager'].includes(role))
    ...
    admin: {
      group: 'Operations',
      useAsTitle: 'name',
      defaultColumns: ['name', 'alias', 'type', 'status', 'capacity', 'updatedAt'],
    },
  },
  essentialFields,
  [
